...
<div id="c2bot" style="background:#f6faf2;border:1px solid #dbe8c8;border-radius:20px;padding:20px;max-width:1100px;margin:auto;font-family:Arial,sans-serif;">

<h2>🤖 Kapitel 2 Lernbot</h2>

<div id="c2-output" style="background:white;border:1px solid #d8e6c8;border-radius:16px;padding:16px;min-height:180px;max-height:360px;overflow:auto;line-height:1.6;margin-bottom:14px;">
<b>Bot:</b> Hallo! Ask me about Kapitel 2 😊
</div>

<input id="c2-input" placeholder="Type your question here..." style="width:70%;padding:12px;border-radius:12px;border:1px solid #b9cda0;">
<button id="c2-ask" style="padding:12px 16px;border-radius:12px;border:none;background:#7b8b2b;color:white;font-weight:bold;">Ask</button>
<button id="c2-clear" style="padding:12px 16px;border-radius:12px;border:1px solid #7b8b2b;background:white;color:#7b8b2b;font-weight:bold;">Clear</button>

<h3>💡 Quick Questions</h3>

<div id="c2-quick" style="display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:10px;margin-top:12px;"></div>
...
setTimeout(function(){
  ...
  const questions = [
    ...
    "What is a Buddy?",
    "What is Erstsemester?",
    "What does Studium mean?",
    "What are Freizeit activities?",
    "What does der Ball mean?",
    "What is das Formular?",
    "What verbs go with Musik?",
    "What verbs go with Bücher?",
    "What verbs go with Salsa?",
    "What is ein and eine?",
    "What is kein and keine?",
    "When do we use nicht?",
    "When do we use kein?",
    "What are personal pronouns?",
    "What is Akkusativ?",
    "What verbs need Akkusativ?",
    "What is formal email?",
    "What is informal email?",
    "How to say I like sport?"
  ];

  function answer(q){
    q = q.toLowerCase();

    if(q.includes("chapter")) return "Chapter 2 is about Studium und Freizeit. You learn university words, hobbies, sport, articles, negation, pronouns, Akkusativ, and emails.";
    if(q.includes("buddy")) return "A Buddy is a student who helps new students. In German: Ein Buddy begleitet Erstsemester und gibt Tipps.";
    if(q.includes("erstsemester")) return "Erstsemester means first-semester student. They are new at the university.";
    if(q.includes("studium")) return "Das Studium means university studies. Example: Das Studium ist interessant.";
    if(q.includes("freizeit")) return "Freizeit means free time. Examples: Musik hören, Bücher lesen, Sport treiben, Schach spielen.";
    if(q.includes("ball")) return "der Ball means the ball. Example: Ich spiele Ball.";
    if(q.includes("formular")) return "das Formular means form. ein Formular ausfüllen means to fill out a form.";
    if(q.includes("musik")) return "Correct phrase: Musik hören. Example: Ich höre Musik.";
    if(q.includes("bücher") || q.includes("bucher")) return "Correct phrase: Bücher lesen. Example: Ich lese Bücher.";
    if(q.includes("salsa")) return "Correct phrase: Salsa tanzen. Example: Ich tanze Salsa.";
    if(q.includes("ein") || q.includes("eine")) return "ein/eine means a/an. Examples: ein Job, ein Studium, eine Universität.";
    if(q.includes("kein") || q.includes("keine")) return "kein/keine means no/not a. Examples: kein Student, keine Professorin.";
    if(q.includes("nicht")) return "nicht negates verbs, adjectives, places, or full sentences. Example: Pablo kommt nicht aus Portugal.";
    if(q.includes("pronoun")) return "Pronouns: er = he, sie = she/they, es = it. Example: Der Job ist neu. Er ist interessant.";
    if(q.includes("akkusativ")) return "Akkusativ is used after verbs like haben, brauchen, suchen, kaufen. Masculine changes: ein Computer → einen Computer.";
    if(q.includes("verbs need")) return "Akkusativ verbs in this chapter: haben, brauchen, kaufen, suchen.";
    if(q.includes("formal")) return "Formal email: Sehr geehrter Herr ... / Sehr geehrte Frau ... and Mit freundlichen Grüßen.";
    if(q.includes("informal")) return "Informal email: Hallo ... and Viele Grüße.";
    if(q.includes("sport")) return "You can say: Ich treibe gern Sport. Negative: Ich treibe nicht gern Sport.";

    return "Ask me about Kapitel 2: Studium, Freizeit, Buddy, articles, negation, pronouns, Akkusativ, hobbies, or emails.";
  }
...
|  | Wie ist deine / Ihre Handynummer? | Mein Handynummer ist 0169 – 2831572. |
